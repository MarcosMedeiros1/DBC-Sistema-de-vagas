...
const reprovarCandidato = async (idCandidato) => {
  const confirmacao = confirm("Confirmar reprovação do candidato?");
  ...
  if (confirmacao) {
    ...
    vagaSelecionada.candidatos.forEach((v) => {
      if (v.idCandidato === candidatoReprovado.id) {
        v.reprovado = true;
      }
    });
